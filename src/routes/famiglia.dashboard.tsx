...
function FamilyDashboard() {
  ...
  const allServices = Array.from(new Set(data?.professionals?.flatMap((p: any) => p.services || []) || []));
  const allCities = Array.from(new Set(data?.professionals?.map((p: any) => p.city).filter(Boolean) || []));
  const allExperiences = Array.from(new Set(data?.professionals?.map((p: any) => p.experience).filter(Boolean) || []));
  
  const filteredProfessionals = data?.professionals?.filter((pro: any) => {
    const matchesSearch = !searchQuery || pro.full_name?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesService = selectedService === "Tutti" || (pro.services && pro.services.includes(selectedService));
    const matchesCity = selectedCity === "Tutte" || pro.city === selectedCity;
    const matchesExperience = selectedExperience === "Tutte" || pro.experience === selectedExperience;
          ...
          <div className="bg-primary/5 p-6 md:p-8 rounded-3xl border-2 border-primary/20 mb-10 flex flex-col md:flex-row gap-6 items-center justify-between shadow-sm">
            <div>
              ...
              <p className="text-muted-foreground text-base max-w-xl">
                Scopri le biografie complete, le nazionalità e contatta direttamente i professionisti verificati nella tua zona. Nessun limite, disdici quando vuoi.
              </p>
            </div>
            <div className="w-full md:w-auto shrink-0 flex flex-col items-center gap-2">
              ...
              >
                {checkoutLoading ? <Loader2 className="w-5 h-5 animate-spin mr-2" /> : <Lock className="w-5 h-5 mr-2" />}
                Abbonati a 29€
              </Button>
            </div>
          </div>
          ...
          {!data?.locked && data?.matches && data.matches.length > 0 && (
            <div>
              ...
              <p className="text-muted-foreground mb-6">
                Questi professionisti hanno visualizzato il tuo annuncio e sono interessati a lavorare con te. Puoi contattarli direttamente.
              </p>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {data.matches.map((pro: any) => (
                  <div key={pro.id} className="bg-primary/5 rounded-3xl border-2 border-primary/20 p-6 shadow-sm flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4 mb-4 mt-2">
                      <div className="w-16 h-16 bg-primary/20 rounded-full flex items-center justify-center text-xl font-bold text-primary overflow-hidden">
                        {pro.avatar_url ? (
                          <img src={pro.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <div className="mt-auto">
                      <a href={`mailto:${pro.email}`} className="w-full flex items-center justify-center h-10 px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm font-medium hover:bg-primary/90 transition-colors">
                        Contatta via Email
                      </a>
                    </div>
                  </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {filteredProfessionals.length === 0 && (
                <div className="col-span-full bg-background p-10 rounded-3xl border border-border/60 text-center">
                  ...
                  <p className="text-muted-foreground">Modifica i filtri di ricerca o torna a controllare più tardi.</p>
                </div>
              )}
            </div>
          </div>
